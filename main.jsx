/* ===================== EconoLearn - main.jsx (dropdown portal + size tweaks + analytics fix) ===================== */
...
/* ----------------- UI helpers ----------------- */
const glassCard = "relative rounded-3xl p-6 bg-white/30 backdrop-blur-xl border border-white/40";
...
const Confetti = ({ on }) => {
  useEffect(() => {
    ...
    const p = Array.from({ length: N }).map(() => {
      const ang = Math.random() * Math.PI - Math.PI / 2, sp = 6 + Math.random() * 8;
    ...
    const tick = () => {
      ...
      p.forEach(o => {
        o.vx *= D; o.vy = o.vy * D + G; o.x += o.vx; o.y += o.vy; o.r += o.vr; o.life++;
...
/* ----------------- FancySelect (portal + scroll + auto flip) ----------------- */
function FancySelect({ value, onChange, options }) {
  const [open, setOpen] = useState(false);
  const [pos, setPos] = useState({ left: 0, top: 0, width: 0, placement: 'bottom' });
  ...
  const recalc = () => {
    ...
    const menuH = Math.min(260, Math.round(window.innerHeight * 0.6)); // max list height
    const placement = below >= menuH + 10 ? 'bottom' : 'top';
    const top = placement === 'bottom' ? (r.bottom + 8) : (r.top - menuH - 8);
    setPos({ left: r.left, top, width, placement, menuH });
  };
      ...
      {open && ReactDOM.createPortal(
        <ul
          ...
          style={{
            ...
            top: `${pos.top}px`,
            width: `${pos.width}px`,
            maxHeight: `${pos.menuH || 260}px`,
            overflowY: 'auto',
            zIndex: 10000
          }}
          className="rounded-xl border bg-white/95 backdrop-blur shadow-xl"
        >
          {options.map(opt => (
            <li
              key={opt}
...
const App = () => {
  ...
  const startTimer = (sec) => {
    ...
    timer.current = setInterval(() => {
      setRemaining(p => { if (p <= 1) { clearInterval(timer.current); setPage('result'); return 0; } return p - 1; });
    }, 1000);
  };
                ...
                <div className="mt-5 flex flex-col md:flex-row md:items-end gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-28 sm:w-32 p-2 border rounded-lg bg-white/60 backdrop-blur"
                    />
                  ...
                  <div className="md:ml-auto">
                    ...
                    <div className="p-2 border rounded bg-white/60 backdrop-blur text-sm w-28 sm:w-32 text-center">
                      {fmt(est)}
                    </div>
                  </div>
                </div>
              ...
              {sorted.map(a => (
                <details key={a.id} className="rounded-xl border bg-white/70 backdrop-blur p-4">
                  <summary className="cursor-pointer flex items-center justify-between">
                    <div>
                      <div className="font-semibold">{a.timestamp ? new Date(a.timestamp).toLocaleString() : 'Unknown time'} • {a.mode || '—'} • {a.chapter || '—'}</div>
                      <div className="text-sm text-gray-700">Score: {a.score ?? '—'}/{a.total ?? '—'} ({a.percent ?? 0}%) {a.durationSec ? `• Time: ${fmt(a.durationSec)}` : ''}</div>
                    </div>
                  </summary>
                  <div className="mt-3 space-y-2">
                    {(Array.isArray(a.questions) ? a.questions : []).map((q, i) => {
  ...
  /* ---- ANALYTICS (robust to old entries) ---- */
  if (page === 'analytics') {
    let rows = [];
    try {
      const hist = store.get() || [];
      const agg = {};
      for (const at of hist) {
        if (!at || !Array.isArray(at.questions)) continue;
        const ans = Array.isArray(at.answers) ? at.answers : [];
        at.questions.forEach((q, i) => {
          const ch = (q && q.chapter) ? q.chapter : 'Unknown';
          if (!agg[ch]) agg[ch] = { correct: 0, total: 0 };
          agg[ch].total++;
          if (ans[i] === q.answer) agg[ch].correct++;
        });
      }
      rows = Object.entries(agg).map(([ch, { correct, total }]) => ({ ch, correct, total, pct: total ? Math.round(correct / total * 100) : 0 }))
